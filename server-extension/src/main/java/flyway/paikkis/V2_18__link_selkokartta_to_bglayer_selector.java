...
public class V2_18__link_selkokartta_to_bglayer_selector implements JdbcMigration {
    ...
    private static final String EXTERNAL_ID = "base_selko";
    ...
    public void migrate(Connection connection) throws Exception {
        ...
        updateLayerExternalId(connection, selkokarttaId, EXTERNAL_ID);

        updateViewsWithBaselayerSelector(connection, EXTERNAL_ID);
    }
    ...
    private void updateLayerExternalId(Connection conn, int layerId, String externalId) throws SQLException {
        final String sql = "UPDATE oskari_maplayer SET externalid=? WHERE id=?";
        try (PreparedStatement statement = conn.prepareStatement(sql)) {
    ...
    private void updateViewsWithBaselayerSelector(Connection conn, String externalId) throws SQLException {
        final String sql = "SELECT view_id, seqno, config FROM portti_view_bundle_seq WHERE bundleinstance = 'mapfull'";
        try (PreparedStatement statement = conn.prepareStatement(sql)) {
            try (ResultSet rs = statement.executeQuery()) {
                while (rs.next()) {
                    JSONObject config = JSONHelper.createJSONObject(rs.getString("config"));
                    updateOneViewIfNeeded(conn, rs.getInt("view_id"), rs.getInt("seqno"), config, externalId);
                }
            }
        }
    }

    private void updateOneViewIfNeeded(Connection conn, int viewId, int seqNo, JSONObject config, String externalId) throws SQLException {
            ...
            try {
                ...
                if (PLUGIN_ID.equals(JSONHelper.getStringFromJSON(plugin, "id", null))) {
                    ...
                    for (int j = 0; j < baseLayers.length(); j++) {
                        if (externalId.equals(baseLayers.getString(j))) {
                            hasSelkoLayer = true;
                        }
                    }
                    if (hasSelkoLayer) {
